...
const Home = ({ sessions }) => {
  ...
  const { data, mutate } = useSWR(
    `{
      ...
      cupons(offset:${actualPosts}, limit: 5, order_by: {created_at: desc}) {
        id
        is_percentage
        discount
        description
  ...
  const changeActive = async (id, actual) => {
    try {
      await axios.post(
        ...
        {
          query: `mutation {
            update_cupons_by_pk(pk_columns: {id: "${id}"}, _set: {is_active: ${!actual}}) {
          id
        }
      }`,
        },
  ...
  const changeIsPercentage = async (id, actual) => {
    try {
      await axios.post(
        process.env.NEXT_PUBLIC_PREFIX +
          process.env.NEXT_PUBLIC_SITE_URL +
        ...
        {
          query: `mutation {
            update_cupons_by_pk(pk_columns: {id: "${id}"}, _set: {is_percentage: ${!actual}}) {
          id
        }
      }`,
        },
              ...
              <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                  ...
                  <p className="mt-2 text-sm text-gray-700">
                    Uma listagem de todas os cupons
                  </p>
                </div>
                <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                  <Link href="/adm/cupom/create">
                ...
                <div>
                  {data && (
                    <Pagination
                      ...
                      current={pagination}
                      pageSize={5}
                      total={data.cupons_aggregate.aggregate.count}
                    />
                  )}
                </div>
              </div>
