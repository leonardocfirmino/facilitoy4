...
export default function CreateBanner({ sessions }) {
  ...
  const { data, mutate } = useSWR(
    `{
      ...
      home_recomendados(where: {id: {_eq: "${router.query.edit}"}}) {
        product{
          name
          id
          product_images {
      ...
      await axios.post(
...

        {
          query: `mutation MyMutation2 {
            ...
            update_home_recomendados_by_pk(pk_columns: {id: "${router.query.edit}"}, _set: {position: ${form.target.position.value}, product_id: "${form.target.produto.value}"}) {
              id
            }
        }`,
        },
    ...
    await axios.post(
...

      {
        query: `mutation MyMutation2 {
          ...
          delete_home_recomendados_by_pk(id: "${router.query.edit}") {
            id
          }
      }`,
      },
              ...
              <div className="w-full  items-start">
                <div className="w-full   flex flex-col justify-center pb-4">
                  ...
                  <Select
                    ...
                    required
                    defaultValue={{
                      value: data.home_recomendados[0].product.id,
                      label: (
                        <div className="flex gap-4  items-center">
                          <img
                            className="w-8 h-8"
                            src={
                              "https://space-facilitoy.sfo3.cdn.digitaloceanspaces.com/" +
                              data.home_recomendados[0].product
                                .product_images[0]?.src
                            }
                          />
                          <h1>{data.home_recomendados[0].product.name}</h1>
                        </div>
                      ),
                    }}
                    options={products}
              ...
              <div className="w-full  items-start">
                <div className="w-full   flex flex-col justify-center pb-4">
                  ...
                  <input
                    ...
                    type="number"
                    required
                    defaultValue={data.home_recomendados[0].position}
